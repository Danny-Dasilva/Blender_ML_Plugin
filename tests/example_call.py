...
def batch_render(file_prefix="render"):
    ...
    scene_setup_steps = 2000
    value = True
    loop_count = 0
    labels = []
    while loop_count != scene_setup_steps:
        ball1 = bpy.data.objects["ball1"]
        ball2 = bpy.data.objects["ball2"]
        ball3 = bpy.data.objects["ball3"]
        ball4 = bpy.data.objects["ball4"]
        ball5 = bpy.data.objects["ball5"]
        ball6 = bpy.data.objects["ball6"]
        ball_lst = [ball1, ball2, ball3, ball4, ball5, ball6]
        scene, camera = randomize_camera(4, -5, 1.2)
        randomize_objs(ball_lst, 4, -5, 1.8)
        nearest_ball = find_nearest(camera, ball_lst)
        increment_frames(scene, 50)
#        
        distance, z = center_obj(camera, nearest_ball.matrix_world.to_translation())

        offset(scene, camera, 60)
        value, percent = get_raycast_percentage(scene, camera, nearest_ball, 40)
        if value == False:
            loop_count -= 1
            value = True
        else:
            ...
            objects = get_raycast_percentages(scene, camera, ball_lst, 40)
            print(objects)
